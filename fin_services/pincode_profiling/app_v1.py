...
if(pincode_true==1):
    ...
    temp_member_group_list = dfData[(dfData['district']==select_district)&(dfData['tier']==select_tier)].member_group.unique()
    select_member =  row0_1.selectbox("Select Member Group: ", options=temp_member_group_list,index=0)

    temp_ticket_size_list = dfData[(dfData['district']==select_district)&(dfData['tier']==select_tier)&(dfData['member_group']==select_member)].ticket_size_bb.unique()
    select_ticket =  row0_1.selectbox("Select Ticket Size: ", options=temp_ticket_size_list,index=0)

    temp_score_band_list = dfData[(dfData['district']==select_district)&(dfData['tier']==select_tier)&(dfData['member_group']==select_member)&(dfData['ticket_size_bb']==select_ticket)].score_band_bb.unique()
    temp_band_list = ['band_4','band_3','band_2','band_1','low']
    temp_score_band_list_1 = intersection(temp_band_list,temp_score_band_list)#
    select_score_band =  row0_1.selectbox("Select Score Band: ", options=temp_score_band_list_1,index=0)

    temp=dfData[(dfData['district']==select_district)&(dfData['tier']==select_tier)&(dfData['member_group']==select_member)&(dfData['ticket_size_bb']==select_ticket)&(dfData['score_band_bb']==select_score_band)]
...
if temp_group[temp_group['dpd_status_bb']=='90+'].shape[0]>0:
