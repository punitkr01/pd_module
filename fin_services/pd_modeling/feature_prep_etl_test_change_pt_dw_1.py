...
xgb_model = pickle.load(open('/home/ubuntu/fin_services/pd_modeling/dw_out/xgb_cibil_classifier.pkl', "rb"))
curr_date = date.today()
out_loc='/home/ubuntu/fin_services/cibil_data/model_out/'
...
#     print(df.enquiry)
    ...
    if len(df_n)<1:
        return pd.DataFrame({'user_id':[user_id],'date':[curr_date],'enq_purpose':[''],'enq_amount':[0]})
...
def get_all_loan_type_feats(df,df_raw_dpd,loan_type):
...
def get_cibil_feature_table(df_raw_static,df_raw_dpd,df_raw_enquiry):
    ...
    total_email = df_raw_static[df_raw_static['user_id']==0]['total_email'].tolist()[0]
    gender = df_raw_static[df_raw_static['user_id']==0]['gender'].tolist()[0]
    dob = df_raw_static[df_raw_static['user_id']==0]['dob'].tolist()[0]
...
#         total_loans = df_raw_static[df_raw_static['user_id']==row['user_id']]['total_loans'].tolist()[0]
    ...
    total_loans = len(set(df_raw_dpd[ (df_raw_dpd['timestamp_new']<=timestamp_new)
                           &(df_raw_dpd['is_open']==1)]['loan_id'].tolist()))
    closed_loans = len(set(df_raw_dpd[(df_raw_dpd['timestamp_new']<=timestamp_new)
...
#     dpd_provided = row['dpd']
    ...
    ## mm-yyyy 
    new_df = df_raw_dpd
    print(timestamp_new, timestamp_new-pd.DateOffset(months=3))
    ...
    total_enq_till_date = df_raw_enquiry[(df_raw_enquiry['user_id']==user_id) & (df_raw_enquiry['timestamp']<timestamp_new)].shape[0]
    unique_enquiry_purpose = df_raw_enquiry[(df_raw_enquiry['user_id']==user_id) & (df_raw_enquiry['timestamp']<timestamp_new)]['enq_purpose'].nunique()
    total_enq_amt = df_raw_enquiry[(df_raw_enquiry['user_id']==user_id) & (df_raw_enquiry['timestamp']<timestamp_new)]['enq_amount'].sum()
    new_df['dpd'] = new_df['modified_dpd'] 
#     loan_type = df_raw_dpd[]
    

    df_feature = pd.DataFrame({
        'user_id':[user_id] , 'timestamp':[timestamp] ,'datetime_formatted' : [timestamp_new], 'cibil_score' : [cibil_score], 
        ...
        'sanc_amount':[sanc_amount],'total_loans' : [total_loans], 'total_phone_nos' : [total_phone_nos], 
    ...
    dfs = pd.concat(dfs)
    dfs['total_enq_till_date_on_user_id_enq_purpose'] = total_enq_till_date
    dfs['unique_enquiry_purpose_on_user_id_enq_purpose'] = unique_enquiry_purpose
    dfs['total_enq_amt_on_user_id_enq_purpose'] = total_enq_amt
    for item in list(feat_dict.keys()):
        feat_df= get_all_loan_type_feats(new_df,df_raw_dpd,item)
        #print(feat_df)
        dfs = dfs.merge(feat_df,on=['user_id','datetime_formatted'],how='left')
    dfs['enquiry_purpose'] = df_raw_enquiry[df_raw_enquiry['user_id']==user_id]['enq_purpose'].tolist()[0]
    return dfs
...
def compute_pd(pd_filename):
    ...
    #print(f'df_Statis.shape = {df_static.shape}')
    df_raw_dpd = get_dpd_raw_table(dfs)
    print(f'df_raw_dpd.shape = {df_raw_dpd.shape}')
    ...
    #     df_cibil_feature = df_cibil_feature.apply(get_enquiry_type,axis=1)
       ...
       'last_36_months_dpd']]#.groupby(['user_id','datetime_formatted']).sum().reset_index()
        ...
        f_all.columns =['user_id','datetime_formatted', 'last_3_months_dpd_all', 'last_6_months_dpd_all', 'last_12_months_dpd_all',  'last_36_months_dpd_all']
    ...
    #     print(X.dtypes)
        ...
        predicted_class = xgb_model.predict(X)[0]
        ...
        #df_x['output'] = xgb_model.predict(X)[0]
#         print(dfs)
        #dfs.to_csv(out_l,index=False)
        return df_x
    else:
        print('No DPD info ! Data insufficient to predict ! ')
...
if __name__ == "__main__":
    ...
    #     df_cibil_feature = df_cibil_feature.apply(get_enquiry_type,axis=1)
        ...
        X = features[['cibil_score', 'total_email',
           'gender', 'age', 'open_loans', 'closed_loans', 'total_address','sanc_amount',
            'total_phone_nos', 
            #'total_enq_till_date', 'unique_enquiry_purpose',
           #'total_enq_amt', 
