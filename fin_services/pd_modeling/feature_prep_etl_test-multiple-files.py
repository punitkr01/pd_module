...
import argparse
import pickle
xgb_model = pickle.load(open('dw_out/xgb_cibil_classifier.pkl', "rb"))
...
#     loan_type = df_raw_dpd[]
    ...
    for item in list(feat_dict.keys()):
        feat_df= get_all_loan_type_feats(new_df,item)
        print(feat_df)
        dfs = dfs.merge(feat_df,on=['user_id','datetime_formatted'],how='left')
    dfs['enquiry_purpose'] = df_enquiry[df_enquiry['user_id']==user_id]['enq_purpose'].tolist()[0]
    return dfs
...
#     args = parser.parse_args()
    all_csv = glob.glob('/home/ubuntu/fin_services/cibil_data/parsed_data/*.csv')
    for csv in all_csv:
