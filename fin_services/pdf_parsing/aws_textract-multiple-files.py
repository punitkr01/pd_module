...
#     print(final_enq_list[0]+5,enq_end_idx)
    ...
    return final_df
import argparse
out_loc = '/home/ubuntu/fin_services/cibil_data/train/new_cibil_out/'
...
#     args = parser.parse_args()
    ...
    data_inputs = glob.glob('/home/ubuntu/fin_services/cibil_data/train/new_cibil_pdf/*')
    print(len(data_inputs))
    files_done = glob.glob('/home/ubuntu/fin_services/cibil_data/train/new_cibil_out/*')
    fnames_all_pdf_new = [item.split('/')[::-1][0] for item in data_inputs ]
    fnames_files_done = [item.split('/')[::-1][0].replace('.csv','.PDF') for item in files_done ]
    print(len(files_done))
    final_files_names = list(set(fnames_all_pdf_new)-set(fnames_files_done))
    final_list = ['/home/ubuntu/fin_services/cibil_data/train/new_cibil_pdf/'+item for item in final_files_names]
    print(len(final_list))
    errors=[]
    for input_file in final_list:
        
        df_path = extract_text_and_save_file(input_file)
        if len(df_path)==1:
            try:
            ...
            errors.append(df_path)
    df_error = pd.DataFrame({'error_path':[errors]})
    df_error.to_csv('/home/ubuntu/fin_services/cibil_data/train/errors.csv',index=False)  
